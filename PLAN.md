# PLAN.md — sem_debug Shipping Build Plan
...
```
and add an inline comment at line 16 (`input_passages: list[Passage] = []`) noting that `Passage` is imported to serve this type annotation.
**Why:** `Match` is never referenced in the file. `Passage` is required only for the type annotation on line 16.
**Verification:** `pytest -x` must pass.
...
5. Any `?` characters in the terminal where en dashes should appear? (Known Windows console limitation — report whether it obstructs readability.)
...
6. **Task 3 (pyproject.toml) is the only new file creation.** All other tasks are edits or manual runs.
7. **If at any point test count changes from 51 without user-expected reason, flag it as a warning even if tests pass.**
